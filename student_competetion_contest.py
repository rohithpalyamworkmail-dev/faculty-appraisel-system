...
class studentCompetetionContest:
    def __init__(self):
        self.db=ActivityDatabase("competetion_contest__by_students")
    ...
    def calculateCredits(self,students,participation_type,result,prize_position):
        ...
        if participation_type=="International":
            if result=="Prize Winning":return count*{1:4,2:3,3:2}.get(prize_position,0)
            return count*0.4
        return 0
    ...
    def edit_document(self):
        try:
            ...
            if df.empty:
                st.info("No Competition/Contest entries are available for editing.")
                return
            ...
            config={"name_roll_number":st.column_config.MultiselectColumn("Students",options=students,required=True),"batch":"Batch","participated_in":st.column_config.SelectboxColumn("Participated In",options=["competetion","Contest"],required=True),"result":st.column_config.SelectboxColumn("Result",options=["Participation","Prize Winning"],required=True),"participation_type":st.column_config.SelectboxColumn("Participation Type",options=["National","International"],required=True),"team_name":"Team Name","proof_url":"Proof URL","prize_position":st.column_config.SelectboxColumn("Prize Position",options=[0,1,2,3],required=True),"awarded_credits":"Awarded Credits"}
            edited_df=st.data_editor(df,num_rows="delete",use_container_width=True,hide_index=True,disabled=["batch","awarded_credits"],column_config=config,key="student_competetion_editor")

            if st.toggle("Update",key="student_competetion_update"):
                rows=[]
                for _,row in edited_df.iterrows():
                    selected=row["name_roll_number"] if isinstance(row["name_roll_number"],list) else [x.strip() for x in str(row["name_roll_number"]).split(",") if x.strip()]
                    result=row["result"];position=0 if result=="Participation" else int(row["prize_position"] or 0)
                    credits=self.calculateCredits(selected,row["participation_type"],result,position)
                    rows.append({"name_roll_number":",".join(selected),"batch":row["batch"],"participated_in":row["participated_in"],"result":result,"participation_type":row["participation_type"],"team_name":row["team_name"],"proof_url":row["proof_url"],"prize_position":position,"awarded_credits":credits})

                if self.db.replace_pending(pd.DataFrame(rows)):st.success("Competition/Contest Updated Successfully.")
    ...
    def viewDocuments(self):
        try:
            ...
            if df.empty:
                st.info("No Competition/Contest entries found.")
                return

            st.subheader("Student Competition / Contest")
    ...
    def main_layout(self):
        ...
        with tab_entry:
            ...
            with col1:
                ...
                participated_in=st.pills("Participated In",["competetion","Contest"],selection_mode="single",wrap=True)
                ...
                if st.toggle("Check",key="student_competetion_check"):
                    ...
                    elif not participated_in:st.warning("Please select Competition or Contest.")
